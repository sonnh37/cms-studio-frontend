...
export default function Page({params}: { params: { albumId: string } }) {
    ...
    const fetchPhotos = async () => {
        try {
            const response = await axios.get(`https://localhost:7192/albums/${params.albumId}`);
    ...
    const handlePhotoSelectionChange = async (photo: Photo, checked: boolean) => {
        ...
        if (checked) {
            try {
                await axios.post(`https://localhost:7192/albums/${params.albumId}/photos`, photo);
    ...
    const handleDeletePhoto = async (photo: Photo) => {
        try {
            ...
            await axios.delete(`https://localhost:7192/albums/${params.albumId}/photos`, {
                params: {
                    ...
                    {photos
                        ...
                        .map((photo) => (
                            <TableRow key={photo.id}>
                                <TableCell>
                                    {photo.src &&
                                        <img src={photo.src} alt={photo.title} className="w-12 h-12 object-cover"/>}
                                </TableCell>
